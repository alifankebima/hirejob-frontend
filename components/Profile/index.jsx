...
const Profile = (props) => {
  return (
    <div className="col-sm-12 col-md-3 p-4 bg-white shadow-sm rounded d-flex flex-column">
      <img src={props.image} crossOrigin="anonymous" className="img-fluid rounded-circle align-self-center" alt="Profile" />
      ...
      {!!props.hireButton && <Link href={`/worker/${props.id}/hire`} ><Button text="Hire" className="mt-3 w-100"/></Link>}
      <div className='mt-3 fs-5'>Skill</div>
      <div className='d-flex flex-wrap mt-2'>
        {props.skill && props.skill.map((data) =>
        <div className="badge text-bg-warning text-white mb-2 me-2">{data.name}</div>
      )}
      </div>
    </div>
  )
